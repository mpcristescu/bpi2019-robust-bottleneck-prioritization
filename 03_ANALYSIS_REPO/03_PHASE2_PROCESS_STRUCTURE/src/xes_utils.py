...
def val(attrs, key, default=None):
    return attrs.get(key, (None, default, None))[1]

def resource_kind(resource) -> str:
    r = "" if resource is None else str(resource).strip()
    if r.startswith("user_"):
        return "human_user"
    if r.startswith("batch_"):
        return "batch_user"
    if r.upper() in {"", "NONE", "NULL", "N/A", "NA"}:
        return "no_recorded_user"
    return "other_resource_value"
